...
class ButterFly {
    ...
    public flatWings: Array<FlatWing>;
    public Wing:Wing;
    public lines;
    ...
    constructor() {
        ...
        for (let j of LinesOnWings) {
            let centerPoint = [
                ...this.flatWings[j].vertices.slice(0, 2),
                this.flatWings[j].vertices[2] * 1.5
            ];
            for (let i = 0; i < this.flatWings[j].vertices.length; i += 3) {
                this.lines.push(...centerPoint);
                this.lines.push(...this.flatWings[j].vertices.slice(i, i + 2), centerPoint[2]);
            }
        }
