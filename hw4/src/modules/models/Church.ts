...
export class Church extends Translatable implements Drawable{
    ...
    _gl.uniformMatrix4fv(gl.programInfo.uniformLocations.modelViewMatrix, false, flatten(mat4()));
        ...
        for (let i in this.building) {
            ...
            _gl.vertexAttribPointer(gl.programInfo.attribLocations.normalPosition, 3, _gl.FLOAT, false, 0, 0);
            _gl.drawArrays(_gl.TRIANGLE_FAN, 0, this.building[i].normals.length / 3)
        ...
        for (let i in this.roof) {
            ...
            _gl.vertexAttribPointer(gl.programInfo.attribLocations.normalPosition, 3, _gl.FLOAT, false, 0, 0);
            _gl.drawArrays(_gl.TRIANGLE_FAN, 0, this.roof[i].normals.length / 3)
        }
        //door
        for (let i in this.door) {
            ...
            _gl.vertexAttribPointer(gl.programInfo.attribLocations.normalPosition, 3, _gl.FLOAT, false, 0, 0);
            _gl.drawArrays(_gl.TRIANGLE_FAN, 0, this.door[i].normals.length / 3)
        }
        //window
        for (let i in this.window) {
            ...
            _gl.vertexAttribPointer(gl.programInfo.attribLocations.normalPosition, 3, _gl.FLOAT, false, 0, 0);
            _gl.drawArrays(_gl.TRIANGLE_FAN, 0, this.window[i].normals.length / 3)
        }
         //wall
        //  for (let i in this.wall) {
